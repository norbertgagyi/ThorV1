...
namespace Thor.DataAccess
{
    public static class Settings
    {
        ...
        private static string[] fisiere = { "FACTURI.dbf", "PARTENER.dbf", "BANCA.dbf", "CASA.dbf", "COMPENS.dbf", "MARFAFAC.dbf", "RECEPTII.dbf", "MARFAREC.dbf", "STOCINV.DBF" };
        ...
        public static int NotificariZileAvans { get; set; } = 7;

        public static string CaleBackup { get; set; }
                ...
                if (linii.Length > 0)
                {
                    ...
                    for (int i = 1; i < linii.Length; i++)
                    {
                        if (linii[i].Length != 0)
                        {
                            try
                            {
                                string[] elemente = linii[i].Split("*".ToCharArray());

                                if (!CaiFoldereDBF.ContainsKey(elemente[0]))
                                    CaiFoldereDBF.Add(elemente[0], elemente[1]);
                            }
                            catch { }
                        }
                    }
        ...
        private static string CaleDocumenteFirma(string caleFirma)
        {
            return caleFirma + @"\" + "Documente";
        }
        ...
        public static List<Eveniment> Evenimente_NerezolvateInInterval(DateTime dataInceput, DateTime dataSfarsit)
        {
            //pot fi toate, asa am dat acuma
            return Evenimente_FirmeNotificariActive().FindAll(x => x.Stare == "Nerezolvat" && (x.Data_Eveniment.Date <= dataSfarsit));
        }
        ...
        public static List<Eveniment> Evenimente_Toate_Istoric()
        {
            return Evenimente_Toate().FindAll(x => x.Data_Eveniment <= DateTime.Today);
        }
